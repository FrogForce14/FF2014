...
public class VisionSampleProject2014 {
    ...
    final int HORIZ_ASPECT_RATIO_LIMIT = 55;
    final int VERT_ASPECT_RATIO_LIMIT = 35;  //aspect ratio limit. let me see how that is calculated
    ...
    AxisCamera camera;          // the axis camera object (connected to the switch) this is obvious
	...
	int verticalTargetCount, horizontalTargetCount;
        ...
        while(!connected){
            ...
            try {
                System.out.println("Is like totally trying dood");
                /**
                ...
                //image = new RGBImage("/testImage.png");		// get the sample image from the cRIO flash
                BinaryImage image = colorimage.thresholdHSV(118, 255, 100, 255, 40, 255);   // keep only green objects
                image.write("/BImage.png");
                //thresholdImage.write("/threshold.bmp");
                
                
                
                image = image.convexHull(true);
                image.write("/CHImage.png");
                image = image.particleFilter(cc);           // filter out small particles
                image.write("/FImage.png");
                //image.write("/image.bmp");
                
                //iterate through each particle and score to see if it is a target
                Scores scores[] = new Scores[image.getNumberParticles()];
                horizontalTargetCount = verticalTargetCount = 0;
                
                if(image.getNumberParticles() > 0)
                {
                        System.out.println("EnoughParticles");
			for (int i = 0; i < MAX_PARTICLES && i < image.getNumberParticles(); i++) {
			ParticleAnalysisReport report = image.getParticleAnalysisReport(i);
                        scores[i] = new Scores();
					
			//Score each particle on rectangularity and aspect ratio
			scores[i].rectangularity = scoreRectangularity(report);
			scores[i].aspectRatioVertical = scoreAspectRatio(image, report, i, true);
			scores[i].aspectRatioHorizontal = scoreAspectRatio(image, report, i, false);			
			...
			target.verticalIndex = verticalTargets[0];
                        System.out.println(verticalTargetCount);
			for (int i = 0; i < verticalTargetCount; i++)
			{
                            System.out.println("Ummmm, for loooop");
				ParticleAnalysisReport verticalReport = image.getParticleAnalysisReport(verticalTargets[i]);
				for (int j = 0; j < horizontalTargetCount; j++)
				{
                                    ParticleAnalysisReport horizontalReport = image.getParticleAnalysisReport(horizontalTargets[j]);
                                    double horizWidth, horizHeight, vertWidth, leftScore, rightScore, tapeWidthScore, verticalScore, total;
	
                                    //Measure equivalent rectangle sides for use in score calculation
                                    horizWidth = NIVision.MeasureParticle(image.image, horizontalTargets[j], false, MeasurementType.IMAQ_MT_EQUIVALENT_RECT_LONG_SIDE);
                                    vertWidth = NIVision.MeasureParticle(image.image, verticalTargets[i], false, MeasurementType.IMAQ_MT_EQUIVALENT_RECT_SHORT_SIDE);
                                    horizHeight = NIVision.MeasureParticle(image.image, horizontalTargets[j], false, MeasurementType.IMAQ_MT_EQUIVALENT_RECT_SHORT_SIDE);
                            ...
                            {
                                System.out.println("checking for hot, target count is > 0");
                                    //Information about the target is contained in the "target" structure
                                    //To get measurement information such as sizes or locations use the
                                    //horizontal or vertical index to get the particle report as shown below
                                    ParticleAnalysisReport distanceReport = image.getParticleAnalysisReport(target.verticalIndex);
                                    double distance = computeDistance(image, distanceReport, target.verticalIndex);
                                    if(target.Hot)
                                    {
